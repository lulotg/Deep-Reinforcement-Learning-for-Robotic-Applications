...
if config_group==1:
    ...
    param_env={'incW':0.055,'decRel':50,'rOutMin':0.0,'rOutMax':5,'rInMin':0,'rInMax':1,'penalty':-5,'setRelevanceMap':False,'render':0,'rewardType':1}
    ...
    param_env['render']=1 #New image model
    param={'param_agent':param_agent.copy(),'param_env':param_env.copy()}
    ...
    with open(output_folder+'param_listMulti1.pickle', 'wb') as handle:
  ...
  #This parameterization is similar to the paper (sel_model=0 and render=0, with DQN (frequpdate=0)
    param_env={'incW':0.055,'decRel':50,'rOutMin':0,'rOutMax':10,'rInMin':-255,'rInMax':255,'penalty':-10,'setRelevanceMap':False,'render':1,'rewardType':0}
    param_agent={'freqUpdate':0,'sel_model':1,'batch_size':128,'num_of_episodes':500,'steps_per_episodes':100,'mem_size':10000,'gamma':0.95,'alpha':0.001,'epsilon_max':0.95,'epsilon_step':1e-3}
    ...
    with open(output_folder+'param_listMulti2.pickle', 'wb') as handle:
        pickle.dump(param_list, handle)
    ###########################################################################
