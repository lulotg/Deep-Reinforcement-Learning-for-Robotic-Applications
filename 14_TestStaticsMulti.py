...
import EnvironmentMulti as Environment
# Import agent actions and model.
import AgentMulti as Agent 
...
def run_episode(N,env,model,draw):
    ...
    if draw['simulate_one_case']:
        ...
        with writer.saving(fig, video_file, 100):
            ax1.imshow(state[:,:,0:3]) #To draw, we drop the forth dimension, if exists
            plt.show()
            plt.pause(0.1)
            writer.grab_frame()
...
folder_net='../Data/Multi2/Networks/'
folder_train='../Data/Multi2/TrainingCurves/'
folder_statistics = '../Data/Multi2/Statistics/'
video_folder = "../Data/Multi1/Video/"
...
for k in range(hm):
    ...
    #param_env['render']=0   #Solo se usa para dibujar ahora, asi que aqui el mejor render es el que proponen ellos
    
    
    # Get the environment (this will get all methods).
    env = Environment.environment(param['param_map'],param['N_ASV'],param_env)
    ...
    if asv_param['sel_model']==0:
        stringNet='DQNModelOrig'+str(asv_param['freqUpdate'])
    elif asv_param['sel_model']==1:
        stringNet='DQNModelFer'+str(asv_param['freqUpdate'])
    else:
        stringNet='Dueling'+str(asv_param['freqUpdate'])


    if param_env['rewardType'] == 0:
        stringNet = stringNet + 'rewardType' + str(param_env['rewardType'])
    elif param_env['rewardType'] == 1:
        stringNet = stringNet + 'rewardType' + str(param_env['rewardType'])


    if param_env['render']==1:
        stringNet = stringNet + 'render' + str(param_env['render'])
    elif param_env['render']==2:
        stringNet = stringNet + 'render' + str(param_env['render'])        

    strtime=datetime.now().strftime('-%m-%d-%H-%M')
    file_common=str(k)+'_'+stringNet+strtime
    ...
    if simulate_one_case==False :
        ...
        # Check if the path of folder_net exists. If not, create it.
        if not os.path.isdir(folder_statistics):
            os.makedirs(folder_statistics)
        file = open(folder_statistics+file_common+'.names', 'bw')
        file.close()

        covers.append(cover)
        timemeans.append(timemean)
        # Save the buffers for late retrieval and comparing.
        np.savetxt(folder_statistics+'cover_'+file_common+'.csv', cover, delimiter=",")
        np.savetxt(folder_statistics+'timemean_'+file_common+'.csv', timemean, delimiter=",")
